...
---
# 类与对象的概念  
面向对象编程的主要思想是把构成问题的各个事务分解成各个对象，建立对象的目的不是为了完成一个步骤，而是为了描述一个事物在解决问题中经过的步骤和行为。对象作为程序的基本单元，将程序和数据封装其中，以提高软类（客观对象的抽象对象件的重用性、灵活性和扩展性。类，是创建对象的模板，一个类可以创建多个相同的对象；对象，是的实例，是按昭类的规则创建的。
